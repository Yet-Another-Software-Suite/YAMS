# CRT Absolute Encoder Configuration Guide

This guide shows how to configure the CRT solver (`CRTAbsoluteEncoder` + `CRTAbsoluteEncoderConfig`) so you can recover a unique multi-turn mechanism angle with minimal ambiguity, even with backlash and sensor noise.
...
- All other settings are configured via `CRTAbsoluteEncoderConfig` before passing it to `CRTAbsoluteEncoder`.
...
- **Direct ratio**: `withEncoderRatios(enc1RotPerMechRot, enc2RotPerMechRot)` when you already computed the ratios. Example:
  ```java
  cfg.withEncoderRatios(/* enc1 */ 50.0, /* enc2 */ 18.3333);
  ```
- **Shared drive gear**: `withCommonDriveGear(commonRatio, driveGearTeeth, encoder1PinionTeeth, encoder2PinionTeeth)`
  ...
  - Example: turret gearbox is 12:50 -> 10:110; encoders use 30T and 31T pinions. `commonRatio` is 110.0 / 10.0 = 11.0; shared drive gear is 50T:
    ```java
    cfg.withCommonDriveGear(/* commonRatio */ 11.0,
                            /* driveGearTeeth */ 50,
                            /* encoder1Pinion */ 30,
                            /* encoder2Pinion */ 31);
    ```
- **Gear chain (simple mesh sequence)**: `withAbsoluteEncoder1Gearing(teeth...)` / `withAbsoluteEncoder2Gearing(teeth...)`
  - Teeth listed from mechanism-side gear to encoder pinion. Example:
    ```java
    cfg.withAbsoluteEncoder1Gearing(50, 20, 40); // 50 drives 20, 20 drives 40
    cfg.withAbsoluteEncoder2Gearing(60, 20);     // 60 drives 20
    ```
- **Explicit stages (driver, driven pairs)**: `withAbsoluteEncoder1GearingStages(driver1, driven1, driver2, driven2, ...)` / `withAbsoluteEncoder2GearingStages(...)`
  - Use for compound or same-shaft trains. Example:
    ```java
    cfg.withAbsoluteEncoder1GearingStages(12, 36, 18, 60); // 12->36, then 18->60
    cfg.withAbsoluteEncoder2GearingStages(12, 60);         // single stage 12->60
    ```
- **Inversion**: Configure both in one place:
  ```java
  cfg.withAbsoluteEncoderInversions(/* enc1 inverted */ false,
                                    /* enc2 inverted */ true);
  ```
  If the devices support on-board inversion, set it there and leave this false to avoid double inversion.
...
## Offsets and limits
- **Offsets before wrap**: `withAbsoluteEncoderOffsets(enc1Offset, enc2Offset)` (in rotations) shifts raw readings prior to wrapping into `[0, 1)`. Use after mechanical zeroing and avoid double-offsetting if the device already applies an offset.
...
2) Set offsets so that the zero pose reads the desired mechanism zero after wrap.
...
- Wrong ratio sign: mechanism turns positive but angle decreases -> set encoder inversion (or device inversion) instead of negating in user code.
- Missing a gear stage: forgetting chain or belt stages yields too-small ratios; candidates overlap -> `AMBIGUOUS`.
- Non-coprime pinions with CRT coverage: shared factors shrink coverage; `coverageSatisfiesRange()` becomes false. Pick coprime counts or widen the coverage margin.
- Zeroing at a wrap edge: offsets that place the parked pose near 0 or 1 cause wrap flips from noise. If your zero offset lands within a few hundredths of 0 or 1 rotations, physically re-phase the encoder (pull the pinion or magnet, rotate a tooth or two, and remesh) so your zero sits near mid-window, then reset offsets. Do not rely on software offsets to live on the edge.
...
- Double inversion or offset: applying inversion or offset both on device and in config produces mirrored or shifted angles. Do it once.
- Skipping power-cycle tests: CRT must reconstruct after reboot; always validate cold-start behavior.

## Example configuration (motor-side absolutes with shared drive gear)
```java
// Suppose: mechanism : drive gear = 12:1, drive gear = 50T, encoders use 19T and 23T pinions.
var cfg =
    new CRTAbsoluteEncoderConfig(enc1Supplier, enc2Supplier)
        .withCommonDriveGear(
...
// In simulation, you can inspect:
cfg.getUniqueCoverage();          // Optional<Angle> coverage from prime counts and common scale
cfg.coverageSatisfiesRange();     // Does coverage exceed maxMechanismAngle?
cfg.getRecommendedCrtGearPair();  // Suggested small coprime pair within constraints

// Create the solver:
var crt = new CRTAbsoluteEncoder(cfg);
```

## More gearing examples
- **Dual motor-side absolutes with CRT coverage** (primes auto-set via `withCommonDriveGear`; CRT helpers populated):
  ```java
  var cfg =
      new CRTAbsoluteEncoderConfig(enc1, enc2)
          .withCommonDriveGear(
              /* commonRatio (mech:drive) */ 9.0,
              /* driveGearTeeth */ 48,
              /* encoder1Pinion */ 19,
              /* encoder2Pinion */ 23)
          .withMechanismRange(Rotations.of(-1.0), Rotations.of(1.5))
  ...
  // CRT helpers now have data:
  cfg.getUniqueCoverage();         // populated from primes + commonScaleK
  cfg.coverageSatisfiesRange();    // validate coverage vs travel
  cfg.getRecommendedCrtGearPair(); // Optional<CrtGearPair> in simulation
  ```

- **Motor-side absolutes using explicit stage definitions** (no CRT inputs set by gearing; seed the recommender manually):
  ```java
  var cfg =
      new CRTAbsoluteEncoderConfig(enc1, enc2)
  ...
  cfg.getRecommendedCrtGearPair(); // Now returns Optional<CrtGearPair> in simulation
  ```

- **Mixed motor-side absolutes using chain helpers** (no CRT inputs; coverage helpers stay empty):
  ```java
  var cfg =
      new CRTAbsoluteEncoderConfig(enc1, enc2)
          ...
          .withAbsoluteEncoderInversions(false, false);
  // Coverage helpers remain empty because CRT prime inputs are not provided.
  ```
