...
public class BatterySimTest {
  ...
  /**
   * Allowed slack when asserting that voltage does not rise, to absorb floating point/timing noise
   * without masking a real regression (which showed up as multi-volt jumps).
   */
  ...
  private static Command dutyCycleCommand(SmartMotorController smc, double dutyCycle) {
    return ((SmartMotorControllerTestSubsystem) smc.getConfig().getSubsystem())
  ...
  void testFullyChargedBatteryVoltage() throws InterruptedException {
    ...
    try {
      ...
      assertTrue(
          idleVoltage > 12.0, "A fully charged, unloaded battery should read above nominal 12V.");
