...
public final class SchedulerPumpHelper {
   ...
   * already elapsed as of {@code nowMicros} to run and rearm for their next period. {@link
   * SimHooks#stepTimingAsync(double)} advances the simulated clock and wakes those notifier threads
   * without waiting for them to finish, so polling {@link NotifierDataJNI#getNextTimeout()} until
   * it moves past {@code nowMicros} (or there are no notifiers left to wait on) confirms they've
   * actually drained before we let the caller observe the new simulated state.
   *
