...
public class IndexerMechanism implements Mechanism {
  ...
  /**
   * Runs the indexer at a fixed power until interrupted, then stops it.
   *
   * @param power duty cycle, [-1, 1]
  ...
  /**
   * Runs the indexer at the intaking power. Runs until interrupted.
   *
   * @return the intake command
   */
  public Command intake() {
    return runAt(INDEXER_INTAKING_PERCENT, "Indexer.Intake");
  }

  /**
   * Feeds fuel toward the launcher. Used to launch and to eject. Runs until interrupted.
   *
   * @return the feed command
   */
  public Command feed() {
    return runAt(INDEXER_LAUNCHING_PERCENT, "Indexer.Feed");
  }

  /**
   * Holds the fuel back from the launcher while it spins up. Runs until interrupted.
   *
   * @return the hold back command
   */
  public Command holdBack() {
    return runAt(INDEXER_SPIN_UP_PRE_LAUNCH_PERCENT, "Indexer.HoldBack");
  }

  /**
   * Holds the indexer stopped. Used as the default command, at the lowest priority so any other
   * indexer command can take over.
   *
  ...
  public Command idle() {
    ...
    }).withPriority(Command.LOWEST_PRIORITY).named("Indexer.Stop");
  }
