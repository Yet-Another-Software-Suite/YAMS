...
/**
 * Sets the shooter speed and hood position from the distance to the hub, using WCP's shot map. The
 * v2 port's {@code PrepareShotCommand} subclass is now a coroutine command that requires both the
 * shooter and the hood.
 */
public class PrepareShot {
    ...
    // Shared by every PrepareShot, so there is a single dashboard entry.
    ...
    public PrepareShot(Shooter shooter, Hood hood, Supplier<Pose2d> robotPoseSupplier) {
        ...
        this.command = Command.requiring(shooter, hood)
            .executing(coroutine -> {
                while (true) {
    ...
    /** Tracks the shot until canceled, then stops the shooter. */
