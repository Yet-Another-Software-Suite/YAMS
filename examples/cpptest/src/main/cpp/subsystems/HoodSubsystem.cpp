...
HoodSubsystem::HoodSubsystem() {
  ...
  m_motor.emplace(m_hoodMotor, frc::DCMotor::NEO(1),
                  TalonFXSWrapper::MotorArrangement::NEO, m_motorConfig);
...
void HoodSubsystem::SimulationPeriodic() {
  m_hood->SimIterate();
}

frc2::CommandPtr HoodSubsystem::HoodCmd(double dutycycle) {
  return m_hood->Set(dutycycle);
}

frc2::CommandPtr HoodSubsystem::SysId() {
  return m_hood->SysId(units::volt_t{3}, frc2::sysid::ramp_rate_t{3.0},
                       units::second_t{30});
}

frc2::CommandPtr HoodSubsystem::SetAngle(units::degree_t angle) {
  return m_hood->GoToAngle(angle);
}
