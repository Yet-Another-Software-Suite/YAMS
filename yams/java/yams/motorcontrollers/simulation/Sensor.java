...
/**
 * Sensor class using {@link edu.wpi.first.hal.SimDevice}; All fields will use the given supplier on
 * real robots. Fake data is only given when connected to simulation.
 *
 * <p>{@code Sensor} models a named hardware sensor (such as an encoder, limit switch, or analog
 ...
 *
 * <p>On a <b>real robot</b> every field delegates to the real-hardware supplier provided at
 * construction time. In <b>simulation</b>, a {@link edu.wpi.first.hal.SimDevice} is registered with
 * WPILib so the WPILib Glass simulation GUI can read and override individual field values, enabling
 * hardware-in-the-loop-style testing without real hardware.
 *
 * <p>Trigger overrides ({@link #addSimTrigger}) allow simulation suppliers to inject specific
 * sensor readings (e.g. a limit-switch trip) at a given moment during a simulated match, which is
 * useful for automating regression tests.
 *
