 ...
 * </ul>
 *
 * A discontinuity point <b>must</b> be configured via {@link yams.motorcontrollers.SmartMotorControllerConfig#withExternalEncoderDiscontinuityPoint} whenever
 * a {@link com.revrobotics.spark.SparkAbsoluteEncoder} is used as the external encoder.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
public class SparkWrapper extends SmartMotorController {
   ...
   * @param motor {@link DCMotor} controller by the {@link SparkFlex} or {@link SparkMax}. Must be a
   *     brushless motor.
    ...
    if (m_config.getFeedbackSynchronizationThreshold().isPresent()) {
      if (m_sparkAbsoluteEncoder.isPresent()) {
        if (!Rotations.of(m_sparkRelativeEncoder.getPosition())
            .isNear(
                Rotations.of(m_sparkAbsoluteEncoder.get().getPosition()),
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
            ...
            trapProfile -> {
              ...
              if (m_config.getLinearClosedLoopControllerUse()) {
                m_sparkBaseConfig
                    ...
                    .cruiseVelocity(
                        m_config
                            .convertToMechanism(MetersPerSecond.of(trapProfile.maxVelocity))
                            .in(RotationsPerSecond))
                    .maxAcceleration(
                        m_config
                            .convertToMechanism(
                                MetersPerSecondPerSecond.of(trapProfile.maxAcceleration))
                            .in(RotationsPerSecondPerSecond));
              } else {
                m_sparkBaseConfig
                    .closedLoop
                    .maxMotion
                    .cruiseVelocity(trapProfile.maxVelocity)
    ...
    config
        .getOpenLoopRampRate()
        .ifPresent(rate -> m_sparkBaseConfig.openLoopRampRate(rate.in(Seconds)));
    config
        .getClosedLoopRampRate()
        .ifPresent(rate -> m_sparkBaseConfig.closedLoopRampRate(rate.in(Seconds)));
    ...
    for (var closedLoopControlSlot : ClosedLoopControllerSlot.values()) {
      ...
      config
          .getArmFeedforward(closedLoopControlSlot)
          .ifPresent(
              ff -> {
                m_sparkBaseConfig
                    .closedLoop
      ...
      config
          .getElevatorFeedforward(closedLoopControlSlot)
          .ifPresent(
              ff -> {
                m_sparkBaseConfig
                    .closedLoop
      ...
      config
          .getSimpleFeedforward(closedLoopControlSlot)
          .ifPresent(
              ff -> {
                m_sparkBaseConfig
                    .closedLoop
                ...
                m_pid.ifPresent(
                    pidController ->
                        pidController.setTolerance(
                            config.convertFromMechanism(tolerance).in(Meters)));
              } else {
                m_pid.ifPresent(
                    pidController -> pidController.setTolerance(tolerance.in(Rotations)));
              }
            });
    ...
    config
        ...
        .ifPresent(
            lowerLimit -> {
              m_sparkBaseConfig
                  .softLimit
                  .reverseSoftLimit(lowerLimit.in(Rotations))
    ...
    config
        ...
        .ifPresent(
            upperLimit -> {
              m_sparkBaseConfig
                  .softLimit
                  .forwardSoftLimit(upperLimit.in(Rotations))
    ...
    if (config.getSupplyStallCurrentLimit().isPresent()) {
      throw new SmartMotorControllerConfigurationException(
          "Supply current limits are not supported on Sparks",
          "Supply current limit not set",
    ...
    if (config.getContinuousWrapping().isPresent()
      ...
      m_sparkBaseConfig
          ...
          .positionWrappingInputRange(
              config.getContinuousWrappingMin().get().in(Rotations),
    ...
    if (config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder instanceof SparkAbsoluteEncoder) {
        double absoluteEncoderConversionFactor =
            config
        ...
        m_sparkBaseConfig
            .absoluteEncoder
            .positionConversionFactor(absoluteEncoderConversionFactor)
      ...
      for (Pair<Object, Boolean> follower : config.getFollowers().get()) {
        if (follower.getFirst() instanceof SparkMax) {
          ...
          m_config
              ...
              .ifPresent(
                  mode ->
                      f_cfg.idleMode(mode == MotorMode.BRAKE ? IdleMode.kBrake : IdleMode.kCoast));
    ...
    var resetMode =
        m_config.getResetPreviousConfig()
  ...
  public Current getStatorCurrent() {
    return m_simSupplier.isPresent()
        ? m_simSupplier.get().getStatorCurrent()
  ...
  public void setMechanismLimits(Angle lower, Angle upper) {
    ...
    m_sparkBaseConfig
        .softLimit
        .reverseSoftLimit(lower.in(Rotations))
