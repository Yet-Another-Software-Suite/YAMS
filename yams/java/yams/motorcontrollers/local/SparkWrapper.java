...
public class SparkWrapper extends SmartMotorController {
    ...
    if (RobotBase.isSimulation()) {
      ...
      if (!setupRan) {
        ...
        m_dcMotorSim = Optional.of(
            new DCMotorSim(LinearSystemId.createDCMotorSystem(m_motor, m_config.getMOI(),
  ...
  public void seedRelativeEncoder() {
    if (m_sparkAbsoluteEncoder.isPresent()) {
      ...
      sparkRelativeEncoderSim.ifPresent(sparkRelativeEncoderSim
          -> sparkRelativeEncoderSim.setPosition(m_sparkAbsoluteEncoder.get().getPosition()));
    }
  }
  ...
  public void synchronizeRelativeEncoder() {
    if (m_config.getFeedbackSynchronizationThreshold().isPresent()) {
      if (m_sparkAbsoluteEncoder.isPresent()) {
        if (!Rotations.of(m_sparkRelativeEncoder.getPosition())
                .isNear(Rotations.of(m_sparkAbsoluteEncoder.get().getPosition()),
    ...
    if (RobotBase.isSimulation() && m_simSupplier.isPresent()) {
      ...
      m_simSupplier.ifPresent(mSimSupplier -> {
        sparkSim.ifPresent(sim
            -> sim.iterate(mSimSupplier.getMechanismVelocity().in(RotationsPerSecond),
                mSimSupplier.getMechanismSupplyVoltage().in(Volts), simLoop.in(Second)));
        sparkRelativeEncoderSim.ifPresent(sim
            -> sim.iterate(
                mSimSupplier.getMechanismVelocity().in(RotationsPerSecond), simLoop.in(Seconds)));
        m_sparkAbsoluteEncoderSim.ifPresent(absoluteEncoderSim
            -> absoluteEncoderSim.iterate(
                mSimSupplier.getMechanismVelocity().in(RotationsPerSecond), simLoop.in(Seconds)));
      });
  ...
  public void setEncoderPosition(Angle angle) {
    if (m_sparkAbsoluteEncoder.isPresent()) {
      m_sparkBaseConfig.absoluteEncoder.zeroOffset(
          getMechanismPosition().minus(angle).in(Rotations));
      m_sparkAbsoluteEncoderSim.ifPresent(
          absoluteEncoderSim -> absoluteEncoderSim.setPosition(angle.in(Rotations)));
    }
    m_sparkRelativeEncoder.setPosition(angle.in(Rotations));
    sparkRelativeEncoderSim.ifPresent(
        relativeEncoderSim -> relativeEncoderSim.setPosition(angle.in(Rotations)));
    ...
    if (m_expoProfile.isEmpty() && m_lqr.isEmpty() && angle != null) {
      configureSpark(()
                         -> m_sparkPidController.setSetpoint(
                             angle.in(Rotations), m_positionControlType, m_closedLoopSlot));
    }
  ...
  public void setVelocity(AngularVelocity angularVelocity) {
    ...
    if (m_lqr.isEmpty() && angularVelocity != null) {
      configureSpark(()
                         -> m_sparkPidController.setSetpoint(
                             setpointVelocity.orElse(RPM.of(0)).in(RotationsPerSecond),
  ...
  public void setVelocity(AngularVelocity angularVelocity, Force feedforwardForce) {
    ...
    if (m_lqr.isEmpty() && angularVelocity != null && setpointFeedforwardForce.isPresent()) {
      ...
      configureSpark(
          ()
              -> m_sparkPidController.setSetpoint(
                  setpointVelocity.orElse(RPM.of(0)).in(RotationsPerSecond), m_velocityControlType,
                  m_closedLoopSlot, feedforwardVoltage.in(Volts), ArbFFUnits.kVoltage));
      m_looseFollowers.ifPresent(smcs -> {
        for (var f : smcs) {
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (m_expoProfile.isPresent() || m_lqr.isPresent()) {
      System.err.println(
          "====== Spark(" + m_spark.getDeviceId() + ") Using RIO Closed Loop Controller ======");
    ...
    if (config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder instanceof SparkAbsoluteEncoder) {
        double absoluteEncoderConversionFactor = config.getExternalEncoderGearing()
        ...
        if (config.getExternalEncoderDiscontinuityPoint().isPresent()) {
          m_sparkBaseConfig.absoluteEncoder.zeroCentered(
              config.getExternalEncoderDiscontinuityPoint().get().isEquivalent(Rotations.of(0.5)));
        }
    ...
    if (config.getZeroOffset().isPresent() && config.getExternalEncoder().isEmpty()
        && !useExternalEncoder) {
      throw new SmartMotorControllerConfigurationException(
  ...
  public double getDutyCycle() {
    return m_spark.getAppliedOutput(); /* m_simSupplier.map(simSupplier ->
                         simSupplier.getMechanismStatorVoltage().in(Volts) /
  ...
  public Current getStatorCurrent() {
    return m_simSupplier.isPresent() ? m_simSupplier.get().getStatorCurrent()
                                     : Amps.of(m_spark.getOutputCurrent());
  }
  ...
  public Voltage getVoltage() {
    return m_simSupplier.isPresent()
        ...
        : Volts.of(m_spark.getAppliedOutput() * m_spark.getBusVoltage());
  }
  ...
  public AngularVelocity getMechanismVelocity() {
    ...
    return RotationsPerSecond.of(
        sparkSim.map(SparkSim::getVelocity).orElseGet(m_sparkRelativeEncoder::getVelocity));
  }
  ...
  public AngularVelocity getRotorVelocity() {
    return RotationsPerSecond.of(getMechanismVelocity()
            .in(RotationsPerSecond) *m_config.getGearing()
  ...
  public Optional<Angle> getExternalEncoderPosition() {
    return m_sparkAbsoluteEncoder.map(
        absoluteEncoder -> Rotations.of(absoluteEncoder.getPosition()));
  }

  @Override
  public Optional<AngularVelocity> getExternalEncoderVelocity() {
    return m_sparkAbsoluteEncoder.map(
        absoluteEncoder -> RotationsPerSecond.of(absoluteEncoder.getVelocity()));
  }
