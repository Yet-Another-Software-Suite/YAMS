...
public class SparkWrapper extends SmartMotorController {
  ...
  public void setupSimulation() {
    if (RobotBase.isSimulation()) {
      ...
      if (!setupRan) {
        ...
        m_dcMotorSim = Optional.of(new DCMotorSim(LinearSystemId.createDCMotorSystem(m_motor, m_config.getMOI(), m_config.getGearing().getMechanismToRotorRatio()), m_motor));
  ...
  public void seedRelativeEncoder() {
    if (m_sparkAbsoluteEncoder.isPresent()) {
      ...
      sparkRelativeEncoderSim.ifPresent(sparkRelativeEncoderSim -> sparkRelativeEncoderSim.setPosition(m_sparkAbsoluteEncoder.get().getPosition()));
    }
  }
  ...
  public void synchronizeRelativeEncoder() {
    if (m_config.getFeedbackSynchronizationThreshold().isPresent()) {
      if (m_sparkAbsoluteEncoder.isPresent()) {
        if (!Rotations.of(m_sparkRelativeEncoder.getPosition()).isNear(Rotations.of(m_sparkAbsoluteEncoder.get().getPosition()), m_config.getFeedbackSynchronizationThreshold().get())) {
          seedRelativeEncoder();
        }
      }
    }
  ...
  public void setIdleMode(MotorMode mode) {
    ...
    configureSpark(() -> m_spark.configure(m_sparkBaseConfig, ResetMode.kNoResetSafeParameters, DriverStation.isEnabled() ? PersistMode.kNoPersistParameters : PersistMode.kPersistParameters));
  }
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    m_sparkBaseConfig.encoder.positionConversionFactor(positionConversionFactor).velocityConversionFactor(velocityConversionFactor);
    ...
    if (config.getContinuousWrapping().isPresent() && config.getContinuousWrappingMin().isPresent()) {
      m_sparkBaseConfig.closedLoop.positionWrappingInputRange(config.getContinuousWrappingMin().get().in(Rotations), config.getContinuousWrapping().get().in(Rotations)).positionWrappingEnabled(true);
    } else if (config.getContinuousWrapping().isPresent()) {
      m_sparkBaseConfig.closedLoop.positionWrappingMaxInput(config.getContinuousWrapping().get().in(Rotations)).positionWrappingEnabled(true);
    }
    ...
    if (config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder instanceof SparkAbsoluteEncoder) {
        ...
        m_sparkBaseConfig.absoluteEncoder.positionConversionFactor(absoluteEncoderConversionFactor).velocityConversionFactor(absoluteEncoderConversionFactor / 60);
    ...
    if (config.getFollowers().isPresent()) {
      for (Pair<Object, Boolean> follower : config.getFollowers().get()) {
        if (follower.getFirst() instanceof SparkMax) {
          ...
          ((SparkMax) follower.getFirst()).configure(f_cfg, ResetMode.kNoResetSafeParameters, DriverStation.isEnabled() ? PersistMode.kNoPersistParameters : PersistMode.kPersistParameters);

        } else if (follower.getFirst() instanceof SparkFlex) {
          var f_cfg = new SparkFlexConfig().follow(m_spark, follower.getSecond());
          m_config.getIdleMode().ifPresent(mode -> f_cfg.idleMode(mode == MotorMode.BRAKE ? IdleMode.kBrake : IdleMode.kCoast));
          ((SparkFlex) follower.getFirst()).configure(f_cfg, ResetMode.kNoResetSafeParameters, DriverStation.isEnabled() ? PersistMode.kNoPersistParameters : PersistMode.kPersistParameters);
    ...
    return configureSpark(() -> m_spark.configure(m_sparkBaseConfig, resetMode, DriverStation.isEnabled() ? PersistMode.kNoPersistParameters : PersistMode.kPersistParameters));
  }
  ...
  public Voltage getVoltage() {
    return m_simSupplier.isPresent() ? m_simSupplier.get().getMechanismStatorVoltage() : Volts.of(m_spark.getAppliedOutput() * m_spark.getBusVoltage());
  }
  ...
  public AngularVelocity getMechanismVelocity() {
    ...
    return RotationsPerSecond.of(sparkSim.map(SparkSim::getVelocity).orElseGet(m_sparkRelativeEncoder::getVelocity));
  }
  ...
  public void setMotorInverted(boolean inverted) {
    ...
    m_spark.configureAsync(m_sparkBaseConfig, ResetMode.kNoResetSafeParameters, DriverStation.isEnabled() ? PersistMode.kNoPersistParameters : PersistMode.kPersistParameters);
  }
