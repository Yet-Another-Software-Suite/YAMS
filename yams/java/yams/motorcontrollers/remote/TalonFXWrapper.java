...
public class TalonFXWrapper extends SmartMotorController {
  ...
  public TalonFXWrapper(TalonFX controller, DCMotor motor, SmartMotorControllerConfig smartConfig) {
    ...
    if (smartConfig.getVendorConfig().isPresent()) {
      ...
      if (genCfg instanceof TalonFXConfiguration) {
        ...
        throw new SmartMotorControllerConfigurationException("TalonFXConfiguration is the only acceptable vendor config type for TalonFXWrapper", "Vendor config is unable to be applied", ".withVendorConfig(new TalonFXConfiguration())");
      }
    } else {
  ...
  private void setFOC(boolean foc) {
    switch (m_positionReq.getName()) {
      ...
      case "PositionVoltage":
        ...
        break;
      default:
        throw new SmartMotorControllerConfigurationException("TalonFX(" + m_talonfx.getDeviceID() + ") does not support the '" + m_positionReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
    }
  ...
  public boolean useCANdiPWM1() {
    ...
    boolean configured = (m_talonConfig.Feedback.FeedbackSensorSource == FeedbackSensorSourceValue.SyncCANdiPWM1 || m_talonConfig.Feedback.FeedbackSensorSource == FeedbackSensorSourceValue.RemoteCANdiPWM1);
  ...
  public boolean useCANdiPWM2() {
    m_configurator.refresh(m_talonConfig.Feedback);
    boolean configured = (m_talonConfig.Feedback.FeedbackSensorSource == FeedbackSensorSourceValue.SyncCANdiPWM2 || m_talonConfig.Feedback.FeedbackSensorSource == FeedbackSensorSourceValue.RemoteCANdiPWM2);
    if (configured && m_candi.isEmpty()) {
    ...
    if (angularVelocity != null && m_lqr.isEmpty() && setpointFeedforwardForce.isPresent()) {
      ...
      switch (m_velocityReq.getName()) {
        ...
        case "VelocityTorqueCurrentFOC":
          ...
          break;
        case "VelocityVoltage":
          ensureRequest(() -> m_talonfx.setControl(((VelocityVoltage) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage)));
          break;
        default:
          throw new SmartMotorControllerConfigurationException("TalonFX(" + m_talonfx.getDeviceID() + ") does not support the '" + m_velocityReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
      }
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (config.getExternalEncoder().isPresent() && useExternalEncoder) {
      ...
      if (config.getStartingPosition().isPresent()) {
        DriverStation.reportWarning("[WARNING] Starting position is not applied to " + (config.getTelemetryName().isPresent() ? getName() : ("TalonFX(" + m_talonfx.getDeviceID() + ")")) + " because an external encoder is used!", false);
      }
      ...
      m_talonConfig.Feedback.SensorToMechanismRatio = config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio();
      if (config.getExternalEncoder().get() instanceof CANcoder encoder) {
        ...
        config.getExternalEncoderInverted().ifPresent(inversion -> {
          cfg.MagnetSensor.withSensorDirection(inversion ? SensorDirectionValue.Clockwise_Positive : SensorDirectionValue.CounterClockwise_Positive);
        });
      ...
      // Zero offset.
      if (config.getExternalEncoderZeroOffset().isPresent()) {
        DriverStation.reportWarning("[WARNING] Zero offset is not supported in TalonFX(" + m_talonfx.getDeviceID() + ") without external encoder.", false);
      }
      ...
      if (config.getStartingPosition().isPresent()) {
        ...
        if (RobotBase.isSimulation()) {
          m_talonfx.getSimState().setRawRotorPosition(config.getStartingPosition().get().times(config.getGearing().getMechanismToRotorRatio()));
        }
        StatusCode applied;
        int iterations = 0;
        do {
      ...
      // Discontinuity point
      if (config.getExternalEncoderDiscontinuityPoint().isPresent()) {
        DriverStation.reportWarning("[WARNING] Discontinuity point is not supported in TalonFX(" + m_talonfx.getDeviceID() + ") without external encoder.", false);
      }
    }
    ...
    if (config.getEncoderInverted().isPresent()) {
      throw new SmartMotorControllerConfigurationException("Integrated encoder phase cannot be set", "Cannot configure TalonFX!", "withEncoderInverted(false)");
    }
  ...
  public void setMeasurementLowerLimit(Distance lowerLimit) {
    if (m_config.getMechanismCircumference().isPresent() && m_config.getMechanismUpperLimit().isPresent()) {
      ...
      m_talonConfig.SoftwareLimitSwitch.withReverseSoftLimitThreshold(m_config.convertToMechanism(lowerLimit)).withReverseSoftLimitEnable(true);
      forceConfigApply();
      m_looseFollowers.ifPresent(smcs -> {
        for (var f : smcs) {
  ...
  public void setMechanismUpperLimit(Angle upperLimit) {
    ...
    m_talonConfig.SoftwareLimitSwitch.withForwardSoftLimitEnable(true).withForwardSoftLimitThreshold(upperLimit);
    forceConfigApply();
    m_looseFollowers.ifPresent(smcs -> {
      for (var f : smcs) {
  ...
  public void setMechanismLowerLimit(Angle lowerLimit) {
    ...
    m_talonConfig.SoftwareLimitSwitch.withReverseSoftLimitEnable(true).withReverseSoftLimitThreshold(lowerLimit);
    forceConfigApply();
    m_looseFollowers.ifPresent(smcs -> {
      for (var f : smcs) {
