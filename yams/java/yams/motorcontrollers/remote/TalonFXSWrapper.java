 ...
 * used. Set via {@link yams.motorcontrollers.SmartMotorControllerConfig#withExternalEncoderDiscontinuityPoint}.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
public class TalonFXSWrapper extends SmartMotorController {
  ...
  public TalonFXSWrapper(TalonFXS controller, DCMotor motor, SmartMotorControllerConfig smartConfig) {
    ...
    if (smartConfig.getVendorConfig().isPresent()) {
      ...
      if (genCfg instanceof TalonFXSConfiguration) {
        ...
        throw new SmartMotorControllerConfigurationException("TalonFXSConfiguration is the only acceptable vendor config for TalonFXS", "Cannot use supplied vendor config", "withVendorConfig(new TalonFXSConfiguration())");
      }
    } else {
  ...
  private void setFOC(boolean foc) {
    switch (m_positionReq.getName()) {
      ...
      case "PositionVoltage":
        ...
        break;
      default:
        throw new SmartMotorControllerConfigurationException("TalonFXS(" + m_talonfxs.getDeviceID() + ") does not support the '" + m_positionReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
    }
    switch (m_velocityReq.getName()) {
      ...
      case "VelocityVoltage":
        ...
        break;
      default:
        throw new SmartMotorControllerConfigurationException("TalonFXS(" + m_talonfxs.getDeviceID() + ") does not support the '" + m_velocityReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
    }
  }
  ...
  public void setupSimulation() {
    if (RobotBase.isSimulation()) {
      ...
      if (!setupRan) {
        m_dcmotorSim = Optional.of(new DCMotorSim(LinearSystemId.createDCMotorSystem(m_dcmotor, m_config.getMOI(), m_config.getGearing().getMechanismToRotorRatio()), m_dcmotor));
  ...
  public void simIterate() {
    if (RobotBase.isSimulation() && m_simSupplier.isPresent()) {
      ...
      if (m_cancoder.isPresent()) {
        ...
        cancoderSim.setVelocity(m_simSupplier.get().getMechanismVelocity().times(m_config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio()));
        cancoderSim.setRawPosition(m_simSupplier.get().getMechanismPosition().times(m_config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio()).minus(m_config.getExternalEncoderZeroOffset().orElse(Rotations
            .zero())));
      ...
      if (m_candi.isPresent()) {
        ...
        if (useCANdiPWM1()) {
          ...
          candiSim.setPwm1Position(m_simSupplier.get().getMechanismPosition().times(m_config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio()).minus(m_config.getExternalEncoderZeroOffset().orElse(Rotations
              .zero())));
          candiSim.setPwm1Velocity(m_simSupplier.get().getMechanismVelocity().times(m_config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio()));
        } else if (useCANdiPWM2()) {
          candiSim.setPwm2Connected(true);
          candiSim.setPwm2Position(m_simSupplier.get().getMechanismPosition().times(m_config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio()).minus(m_config.getExternalEncoderZeroOffset().orElse(Rotations
              .zero())));
          candiSim.setPwm2Velocity(m_simSupplier.get().getMechanismVelocity().times(m_config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio()));
        }
      }
  ...
  /**
   * Check if {@link CANdi} PWM1 is used as the {@link com.ctre.phoenix6.configs.ExternalFeedbackConfigs#ExternalFeedbackSensorSource} in {@link TalonFXSConfiguration#ExternalFeedback}.
   *
  ...
  public void setVelocity(AngularVelocity angularVelocity) {
    ...
    if (angularVelocity != null && m_lqr.isEmpty()) {
      switch (m_velocityReq.getName()) {
        case "MotionMagicVelocityDutyCycle":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityDutyCycle) m_velocityReq).withVelocity(angularVelocity)));
          break;
        case "MotionMagicVelocityTorqueCurrentFOC":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityTorqueCurrentFOC) m_velocityReq).withVelocity(angularVelocity)));
          break;
        case "MotionMagicVelocityVoltage":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityVoltage) m_velocityReq).withVelocity(angularVelocity)));
          break;
        case "VelocityDutyCycle":
        ...
        case "VelocityTorqueCurrentFOC":
          ensureRequest(() -> m_talonfxs.setControl(((VelocityTorqueCurrentFOC) m_velocityReq).withVelocity(angularVelocity)));
          break;
        case "VelocityVoltage":
  ...
  public void setVelocity(AngularVelocity angularVelocity, Force feedforwardForce) {
    ...
    if (angularVelocity != null && m_lqr.isEmpty() && setpointFeedforwardForce.isPresent()) {
      ...
      switch (m_velocityReq.getName()) {
        case "MotionMagicVelocityDutyCycle":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityDutyCycle) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage.in(Volts) / m_dcmotor.nominalVoltageVolts)));
          break;
        case "MotionMagicVelocityTorqueCurrentFOC":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityTorqueCurrentFOC) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardCurrent)));
          break;
        case "MotionMagicVelocityVoltage":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityVoltage) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage)));
          break;
        case "VelocityDutyCycle":
          ensureRequest(() -> m_talonfxs.setControl(((VelocityDutyCycle) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage.in(Volts) / m_dcmotor.nominalVoltageVolts)));
          break;
        case "VelocityTorqueCurrentFOC":
          ensureRequest(() -> m_talonfxs.setControl(((VelocityTorqueCurrentFOC) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardCurrent)));
          break;
        case "VelocityVoltage":
          ensureRequest(() -> m_talonfxs.setControl(((VelocityVoltage) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage)));
          break;
        default:
          throw new SmartMotorControllerConfigurationException("TalonFXS(" + m_talonfxs.getDeviceID() + ") does not support the '" + m_velocityReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
      }
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    m_config.getTrapezoidProfile().ifPresent(trap -> {
      ...
      if (m_config.getVelocityTrapezoidalProfileInUse()) {
        m_talonConfig.MotionMagic.MotionMagicAcceleration = m_config.getLinearClosedLoopControllerUse() ? m_config.convertToMechanism(Meters.of(trap.maxVelocity)).in(Rotations) : trap.maxVelocity;
        m_talonConfig.MotionMagic.MotionMagicJerk = m_config.getLinearClosedLoopControllerUse() ? m_config.convertToMechanism(Meters.of(trap.maxAcceleration)).in(Rotations) : trap.maxAcceleration;
      } else {
        m_talonConfig.MotionMagic.MotionMagicCruiseVelocity = m_config.getLinearClosedLoopControllerUse() ? m_config.convertToMechanism(Meters.of(trap.maxVelocity)).in(Rotations) : trap.maxVelocity;
        m_talonConfig.MotionMagic.MotionMagicAcceleration = m_config.getLinearClosedLoopControllerUse() ? m_config.convertToMechanism(Meters.of(trap.maxAcceleration)).in(Rotations) : trap.maxAcceleration;
      }
    ...
    if (config.getClosedLoopTolerance().isPresent()) {
      throw new SmartMotorControllerConfigurationException("Closed loop tolerance is not available on TalonFXS", "Cannot set closed loop tolerance on TalonFXS", ".withClosedLoopTolerance");
    }
    ...
    for (var closedLoopControlSlot : ClosedLoopControllerSlot.values()) {
      ...
      if (armFeedforward.isPresent() || elevatorFeedforward.isPresent() || simpleMotorFeedforward.isPresent()) {
    ...
    if (config.getStatorStallCurrentLimit().isPresent()) {
      m_talonConfig.CurrentLimits.withStatorCurrentLimitEnable(true).withStatorCurrentLimit(config.getStatorStallCurrentLimit().getAsInt());
    }
    if (config.getSupplyStallCurrentLimit().isPresent()) {
      m_talonConfig.CurrentLimits.withSupplyCurrentLimitEnable(true).withSupplyCurrentLimit(config.getSupplyStallCurrentLimit().getAsInt());
    }
    // Soft limit
    if (config.getMechanismUpperLimit().isPresent()) {
      m_talonConfig.SoftwareLimitSwitch.withForwardSoftLimitEnable(config.getMotorControllerMode() == ControlMode.CLOSED_LOOP).withForwardSoftLimitThreshold(config.getMechanismUpperLimit().get());
    }
    if (config.getMechanismLowerLimit().isPresent()) {
      m_talonConfig.SoftwareLimitSwitch.withReverseSoftLimitEnable(config.getMotorControllerMode() == ControlMode.CLOSED_LOOP).withReverseSoftLimitThreshold(config.getMechanismLowerLimit().get());
    }
    ...
    if (config.getExternalEncoder().isPresent() && useExternalEncoder) {
      ...
      if (config.getStartingPosition().isPresent()) {
        DriverStation.reportWarning("[WARNING] Starting position is not applied to " + (config.getTelemetryName().isPresent() ? getName() : ("TalonFXS(" + m_talonfxs.getDeviceID() + ")")) + " because an external encoder is used!", false);
      }
      ...
      m_talonConfig.ExternalFeedback.SensorToMechanismRatio = config.getExternalEncoderGearing().orElse(MechanismGearing.kOne).getMechanismToRotorRatio();
      if (config.getExternalEncoder().get() instanceof CANcoder encoder) {
        ...
        config.getExternalEncoderInverted().ifPresent(inverted -> {
          cfg.MagnetSensor.withSensorDirection(inverted ? SensorDirectionValue.Clockwise_Positive : SensorDirectionValue.CounterClockwise_Positive);
        });
      ...
      // Zero offset.
      if (config.getExternalEncoderZeroOffset().isPresent()) {
        DriverStation.reportWarning("[WARNING] Zero offset is not supported in TalonFXS(" + m_talonfxs.getDeviceID() + ") without external encoder.", false);
      }
      // Starting position
      if (config.getStartingPosition().isPresent()) {
        forceConfigApply();
        if (RobotBase.isSimulation()) {
          m_talonfxs.getSimState().setRawRotorPosition(config.getStartingPosition().get().times(config.getGearing().getMechanismToRotorRatio()));
        }
        StatusCode applied;
        do {
      ...
      if (config.getExternalEncoderDiscontinuityPoint().isPresent()) {
        DriverStation.reportWarning("[WARNING] Discontinuity point is not supported in TalonFXS(" + m_talonfxs.getDeviceID() + ") without external encoder.", false);
      }
    }
    ...
    if (config.getEncoderInverted().isPresent()) {
      m_talonConfig.ExternalFeedback.withSensorPhase(config.getEncoderInverted().get() ? SensorPhaseValue.Opposed : SensorPhaseValue.Aligned);
    }
      ...
      if (req instanceof ControlRequest) {
        switch (((ControlRequest) req).getName()) {
          ...
          case "VelocityVoltage":
            ...
            break;
          default:
            throw new SmartMotorControllerConfigurationException("TalonFXS(" + m_talonfxs.getDeviceID() + ") does not support the '" + ((ControlRequest) req).getName() + "' control request!", "Cannot use given control request",
                "withVendorControlRequest()");
        }
      } else {
        throw new SmartMotorControllerConfigurationException("TalonFXS(" + m_talonfxs.getDeviceID() + ") does not support the '" + req + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
      }
    }
  ...
  public void setMeasurementUpperLimit(Distance upperLimit) {
    if (m_config.getMechanismCircumference().isPresent() && m_config.getMechanismLowerLimit().isPresent()) {
      ...
      m_talonConfig.SoftwareLimitSwitch.withForwardSoftLimitThreshold(m_config.convertToMechanism(upperLimit)).withForwardSoftLimitEnable(true);
      forceConfigApply();
      m_looseFollowers.ifPresent(smcs -> {
        for (var f : smcs) {
  ...
  public void setClosedLoopSlot(ClosedLoopControllerSlot slot) {
    ...
    switch (m_positionReq.getName()) {
      ...
      case "PositionVoltage":
        ...
        break;
      default:
        throw new SmartMotorControllerConfigurationException("TalonFX(" + m_talonfxs.getDeviceID() + ") does not support the '" + m_positionReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
    }
    switch (m_velocityReq.getName()) {
      case "MotionMagicVelocityDutyCycle":
      ...
      case "VelocityVoltage":
        ...
        break;
      default:
        throw new SmartMotorControllerConfigurationException("TalonFX(" + m_talonfxs.getDeviceID() + ") does not support the '" + m_velocityReq.getName() + "' control request!", "Cannot use given control request", "withVendorControlRequest()");
    }
    m_looseFollowers.ifPresent(smcs -> {
      for (var f : smcs) {
