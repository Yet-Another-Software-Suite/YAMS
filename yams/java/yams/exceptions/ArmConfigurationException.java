...
/**
 * Thrown by {@link yams.mechanisms.positional.Arm} when a required {@link yams.mechanisms.config.ArmConfig} field is missing or invalid.
 *
 ...
 * called exactly once, that {@code ArmConfig.withHardLimits(Angle, Angle)} provides valid lower and
 * upper bounds, that {@code ArmConfig.withLength(Distance)} is set for simulation, and that a
 * starting angle is supplied via {@code SmartMotorControllerConfig.withStartingPosition(Angle)} or
 * an encoder zero-offset.
 *
