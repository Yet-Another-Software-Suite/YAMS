...
public class TalonFXSWrapper extends SmartMotorController {
   ...
   * @throws IllegalArgumentException                   if motor is not a Minion, NEO, NEO 550, or
   *                                                    NEO Vortex, or if
   *                                                    {@link #applyConfig(SmartMotorControllerConfig)}
  ...
  public void setupSimulation() {
    if (RobotBase.isSimulation()) {
      ...
      if (!setupRan) {
        m_dcmotorSim = Optional.of(new DCMotorSim(Models.singleJointedArmFromPhysicalConstants(m_dcmotor, m_config.getMOI(), m_config.getGearing().getMechanismToRotorRatio()), m_dcmotor));
   ...
   * {@inheritDoc}
   *
   * @implNote Does nothing, the TalonFXS fuses its feedback sources on the device, so there is no
  ...
  public void setVelocity(AngularVelocity angularVelocity, Force feedforwardForce) {
    ...
    if (angularVelocity != null && m_lqr.isEmpty() && setpointFeedforwardForce.isPresent()) {
      ...
      switch (m_velocityReq.getName()) {
        case "MotionMagicVelocityDutyCycle":
          ensureRequest(() -> m_talonfxs.setControl(((MotionMagicVelocityDutyCycle) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage.in(Volts) / m_dcmotor.nominalVoltage)));
          break;
        case "MotionMagicVelocityTorqueCurrentFOC":
        ...
        case "MotionMagicVelocityVoltage":
          ...
          break;
        case "VelocityDutyCycle":
          ensureRequest(() -> m_talonfxs.setControl(((VelocityDutyCycle) m_velocityReq).withVelocity(angularVelocity).withFeedForward(feedforwardVoltage.in(Volts) / m_dcmotor.nominalVoltage)));
          break;
        case "VelocityTorqueCurrentFOC":
   ...
   *                                                    without an LQR controller, if an external
   *                                                    encoder inversion or gearing is set while no
   *                                                    external encoder is used, if the vendor
   *                                                    control request is not a supported Phoenix 6
   ...
   * @throws IllegalArgumentException                   if slot is SLOT_3, since the TalonFXS only
