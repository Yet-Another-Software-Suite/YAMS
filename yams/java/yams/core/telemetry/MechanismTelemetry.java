...
public class MechanismTelemetry {
   ...
   * every motor shares the single
   * {@link #setupTelemetry(String)} call and loop timer for the mechanism, instead of each motor
   * re-running
   * {@link #setupTelemetry(String, SmartMotorController)} and clobbering the previous motor's
   * table/loop-time state.
   *
   * @param subTableName    Name of the child table to nest this motor's telemetry under, relative
   *     to the mechanism's
  ...
  public DoubleConsumer publishDouble(String key, String unit) {
    ...
    Optional<DoubleLogEntry> logEntry = dataLogName.map(prefix
        -> new DoubleLogEntry(
