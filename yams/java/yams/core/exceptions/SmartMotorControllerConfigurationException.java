 ...
 * PID controller ({@code withClosedLoopController()}) <li>Distance-based tolerance setter called on
 * an angular controller (missing {@code withLinearClosedLoopController(true)}) <li>MOI parameters
 * ({@code withMOI(Distance, Mass)}) called with {@code null} length or mass <li>NEO 550 motor used
 * without a stator current limit, or with a limit exceeding 40 A <li>Vendor-specific control
