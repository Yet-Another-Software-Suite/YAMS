...
public class LQRConfig {
  ...
  /**
   * Loop time for the {@link org.wpilib.math.controller.LinearQuadraticRegulator}. Could be faster
   * if using
  ...
  /**
   * Control effort (voltage) tolerance. Decrease this to more heavily penalize control effort, or
  ...
  /**
   * Tolerance. Decrease this to more heavily penalize state excursion, or make the controller
   ...
   *     effort, or make the
   *              controller less aggressive. 12 is a good starting point because that is the
   * (approximate) maximum voltage of a battery.
   ...
   *     control effort, or make
   *               the controller less aggressive. 12 is a good starting point because that is the
   * (approximate) maximum voltage of a battery.
   * @return {@link LQRConfig} for chaining.
   */
   ...
   *
   * @param qelms        Velocity error tolerance. Decrease this to more heavily penalize state
   *     excursion, or make the
   *                     controller behave more starting point because that is the (approximate)
   ...
   *     penalize state
   *                             excursion, or make the controller behave more aggressively. This
   * can be tuned to balance the position and velocity errors.
   * @param qelmsVelocity        Velocity error tolerance, in meters per second. Decrease this to
   ...
   * @return {@link LQRConfig} for chaining.
   */
  public LQRConfig withElevator(Distance qelmsPosition, LinearVelocity qelmsVelocity,
      Distance modelPositionTrust, LinearVelocity modelVelocityTrust, Distance encoderPositionTrust,
   ...
   *
   * @param qelmsPosition        Position error tolerance, in rotations. Decrease this to more
   *     heavily penalize state
   *                             excursion, or make the controller behave more aggressively. This
   * can be tuned to balance the position and velocity errors.
   * @param qelmsVelocity        Velocity error tolerance, in rotations per second. Decrease this to
   *     more heavily
   ...
   * @return {@link LQRConfig} for chaining.
   */
  public LQRConfig withArm(Angle qelmsPosition, AngularVelocity qelmsVelocity,
      Angle modelPositionTrust, AngularVelocity modelVelocityTrust, Angle encoderPositionTrust) {
  ...
  public LinearSystem<?, ?, ?> getSystem() {
    switch (m_type.orElseThrow()) {
      case FLYWHEEL -> {
        return Models.flywheelFromPhysicalConstants(
            m_motor, m_moi.in(KilogramSquareMeters), m_gearing.getMechanismToRotorRatio());
      }
      case ARM -> {
        return Models.singleJointedArmFromPhysicalConstants(
            m_motor, m_moi.in(KilogramSquareMeters), m_gearing.getMechanismToRotorRatio());
      }
      case ELEVATOR -> {
        return Models.elevatorFromPhysicalConstants(m_motor,
            m_elevatorMass.orElseThrow().in(Kilograms),
            m_elevatorDrumRadius.orElseThrow().in(Meters), m_gearing.getMechanismToRotorRatio());
      }
    }
  ...
  public KalmanFilter<?, ?, ?> getKalmanFilter(LinearSystem<?, ?, ?> plant) {
    switch (m_type.orElseThrow()) {
      ...
      {
        return new KalmanFilter<N1, N1, N1>(Nat.N1(), Nat.N1(), (LinearSystem<N1, N1, N1>) plant,
            (Vector<N1>) m_modelStdDevs.orElseThrow(), (Vector<N1>) m_encoderStdDevs.orElseThrow(),
            m_period.in(Seconds));
      }
      case ARM,
          ELEVATOR -> /// 2 modeled states, position and velocity. Inputs are volts. Outputs are
                      /// position.
      {
        return new KalmanFilter<N2, N1, N1>(Nat.N2(), Nat.N1(),
            (LinearSystem<N2, N1, N1>) (plant.slice(0)), (Vector<N2>) m_modelStdDevs.orElseThrow(),
            (Vector<N1>) m_encoderStdDevs.orElseThrow(), m_period.in(Seconds));
      }
    }
    throw new IllegalStateException("Invalid LQR Type");
  }
    ...
    switch (m_type.orElseThrow()) {
      case FLYWHEEL -> /// 1 modeled state, velocity. Inputs are volts. Outputs are velocity.
      {
        return new LinearQuadraticRegulator<N1, N1, N1>((LinearSystem<N1, N1, N1>) plant,
            (Vector<N1>) m_qelms.orElseThrow(), VecBuilder.fill(m_relms.in(Volts)),
            m_period.in(Seconds));
      }
      case ARM,
          ELEVATOR -> /// 2 modeled states, position and velocity. Inputs are volts. Outputs are
                      /// position.
      {
        return new LinearQuadraticRegulator<N2, N1, N1>((LinearSystem<N2, N1, N1>) plant,
            (Vector<N2>) m_qelms.orElseThrow(), VecBuilder.fill(m_relms.in(Volts)),
            m_period.in(Seconds));
      }
    }
    throw new IllegalStateException("Invalid LQR Type");
   ...
   */
  @SuppressWarnings("unchecked")
  public LinearSystemLoop<?, ?, ?> getLoop(LinearSystem<?, ?, ?> plant,
      LinearQuadraticRegulator<?, ?, ?> controller, KalmanFilter<?, ?, ?> observer) {
    switch (m_type.orElseThrow()) {
      case FLYWHEEL -> /// 1 modeled state, velocity. Inputs are volts. Outputs are velocity.
      {
        return new LinearSystemLoop<N1, N1, N1>((LinearSystem<N1, N1, N1>) plant,
            (LinearQuadraticRegulator<N1, N1, N1>) controller, (KalmanFilter<N1, N1, N1>) observer,
            m_maxVoltage.in(Volts), m_period.in(Seconds));
      }
      case ARM,
          ELEVATOR -> /// 2 modeled states, position and velocity. Inputs are volts. Outputs are
                      /// position.
      {
        return new LinearSystemLoop<N2, N1, N1>((LinearSystem<N2, N1, N1>) plant,
            (LinearQuadraticRegulator<N2, N1, N1>) controller, (KalmanFilter<N2, N1, N1>) observer,
            m_maxVoltage.in(Volts), m_period.in(Seconds));
      }
    }
    throw new IllegalStateException("Invalid LQR Type");
