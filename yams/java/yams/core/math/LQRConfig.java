...
public class LQRConfig {
  ...
  public LinearSystem<?, ?, ?> getSystem() {
    switch (m_type.orElseThrow()) {
      case FLYWHEEL -> {
        return Models.flywheelFromPhysicalConstants(m_motor, m_moi.in(KilogramSquareMeters), m_gearing.getMechanismToRotorRatio());
      }
      case ARM -> {
        return Models.singleJointedArmFromPhysicalConstants(m_motor, m_moi.in(KilogramSquareMeters), m_gearing.getMechanismToRotorRatio());
      }
      case ELEVATOR -> {
        return Models.elevatorFromPhysicalConstants(m_motor, m_elevatorMass.orElseThrow().in(Kilograms), m_elevatorDrumRadius.orElseThrow().in(Meters), m_gearing.getMechanismToRotorRatio());
      }
    }
