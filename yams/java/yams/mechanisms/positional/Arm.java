...
public class Arm extends SmartPositionalMechanism {
  ...
  public Command runTo(Angle angle, Angle tolerance) {
    return Commands.runOnce(() -> m_smc.setPosition(angle), m_subsystem).andThen(Commands.waitUntil(isNear(angle, tolerance).debounce(0.1, DebounceType.RISING))).withName(m_subsystem.getName() + " RunTo Angle");
  }
  ...
  public Command runTo(Supplier<Angle> angle, Angle tolerance) {
    return Commands.runOnce(() -> m_smc.setPosition(angle.get()), m_subsystem).andThen(Commands.waitUntil(isNear(angle.get(), tolerance).debounce(0.1, DebounceType.RISING))).withName(m_subsystem.getName() + " RunTo Angle Supplier");
  }
