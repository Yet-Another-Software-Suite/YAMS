...
public class FlyWheel extends SmartVelocityMechanism {
  ...
  public FlyWheel(FlyWheelConfig config, SmartMotorController smc) {
    ...
    if (RobotBase.isSimulation()) {
      m_dcmotorSim =
          Optional.of(new DCMotorSim(LinearSystemId.createDCMotorSystem(dcMotor, smcCfg.getMOI(),
  ...
  public <T> Command run(Supplier<T> velocity) {
    ...
    if (velocity.get() instanceof AngularVelocity) {
      return Commands
          .startRun(m_smc::startClosedLoopController,
              () -> m_smc.setVelocity((AngularVelocity) velocity.get()), m_subsystem)
          .withName(cmdName);
